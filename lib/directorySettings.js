...
// firestore.rules only authorizes admin-write/everyone-read on the
// top-level "config" collection — there's no rule for "settings", so a doc
// there permission-denies every read and write. This was the one line
// wrong in the previous version.
const SETTINGS_DOC = doc(db, "config", "directoryVisibility");
...
export function useDirectorySettings() {
  ...
  const loadOnce = useCallback(async () => {
    try {
      ...
      console.error("directorySettings one-shot read failed — check firestore.rules for the config/ collection:", err);
    } finally {
      setLoaded(true);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        settled = true;
        clearTimeout(watchdog);
        console.error("directorySettings listener error — likely a missing/incorrect Firestore rule for config/{doc}:", err);
        loadOnce();
      }
    );
