...
/**
 * Bangla equivalent of getHospitalLocationLabel(), e.g.
 * "আমতলী, বরগুনা, বরিশাল" — no "Division" suffix in Bangla, matching
 * getAddressLabel()'s Bangla formatting in bdData.js.
 */
export function getHospitalLocationLabelBn(hospital) {
  const { division, district, upazila } = getHospitalGeo(hospital);
  if (!division || !district) return "";

  const place = upazila?.nameBn ?? hospital.location.sourceUpazilaNameBn ?? hospital.location.cityCorporationBn ?? "";

  return place ? `${place}, ${district.nameBn}, ${division.nameBn}` : `${district.nameBn}, ${division.nameBn}`;
}

/**
 * Resolves the best available office/employment location label for a
 * profile's `employment` object, in Bangla where possible.
 * - DGHS/DGFP offices picked from the list: looked up by officeId so the
 *   label reflects the office's *actual* location, not just whatever
 *   division/district/upazila filter happened to be set in the UI.
 * - Manually-entered offices (free text, or non-govt): no structured
 *   location to translate, so we fall back to whatever free text is there.
 */
export function resolveEmploymentAddress(employment) {
  if (!employment) return "";
  if (employment.officeId) {
    const hospital = getHospitalById(employment.officeId);
    if (hospital) return getHospitalLocationLabelBn(hospital);
  }
  if (employment.jobType === "non-govt") return employment.officeLocation || "";
  return "";
}